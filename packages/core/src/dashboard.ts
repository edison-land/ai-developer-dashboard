 ...
 * adapters (Claude Code now; Codex + config seeds in Phase 2), derives the set
 * of project paths, snapshots git for exactly those paths, then merges + sorts.
 */
export class Dashboard {
  ...
  async collect(ctx: CollectContext): Promise<UnifiedProject[]> {
    ...
    // Phase 2: codex adapter + config.toml seeds go here.
